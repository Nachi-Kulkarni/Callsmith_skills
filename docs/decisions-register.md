...
| 2026-08-15 | Skill lifecycle: every invocation ends by suggesting the next command from the after→next table (single source in SKILL.md); per-project state file `callsmith.decisions.md` (append-only decisions, rejections, floor rewrites, mistake trace, next step) makes sessions stateful across context resets; asking rules rewritten for plain language and interactive question tools (C25). |
